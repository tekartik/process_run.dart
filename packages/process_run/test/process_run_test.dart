...
void main() {
  ...
  group('run', () {
    ...
    test('stdin', () async {
      ...
      final processResultFuture = runExecutableArguments(dartExecutable!, [
        echoScriptPath,
        '--stdin',
      ], stdin: inCtrl.stream);
    ...
    test('space in binary', () async {
      ...
      expect(
        (await Shell(
          environment: env,
          verbose: false,
        ).run(shellArgument('space in binary'))).first.stdout.toString().trim(),
        'Hello',
      );
    });
    test('space in path', () async {
      ...
      expect(
        (await Shell(
          environment: env,
          verbose: false,
        ).run('binary')).first.stdout.toString().trim(),
        'Hello',
      );
    });
